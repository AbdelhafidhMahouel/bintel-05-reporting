"""olap_abdelhafidh.py - modified copy of olap_case.py.

OLAP reporting on a data warehouse, extended with a
...
Modified by: Abdelhafidh Mahouel
Date: 2026-08
...
- NEW: Rank the top N individual products by total sales,
  across all regions, categories, and time periods.
...
Output:
- data/reporting/sales_reporting_abdelhafidh.csv
...
OBS:
  This is a copy of olap_case.py, modified to add a new
  Top N Products ranking (Section 2.6a) that was not present
...
# NEW: Renamed with alias so this does not overwrite the
# output produced by the original olap_case.py example.
REPORTING_FILE: Final[Path] = DATA_REPORTING / "sales_reporting_abdelhafidh.csv"
...
# NEW: Number of individual products to include in the
# top-products ranking (see top_products_by_sales below).
TOP_N_PRODUCTS: Final[int] = 5
...
# === Section 2.5a (NEW) DEFINE A TOP-PRODUCTS RANKING FUNCTION ===

# Top-N ranking: identify the individual products with the
# highest total sales, across every region and time period.
#
# This is a different OLAP-style operation than slice or dice:
# instead of fixing a dimension value, it ranks members of a
# dimension (Product) by a measure (TotalSales) and keeps only
# the top N. This is a common "leaderboard" pattern in BI
# reporting - e.g., "What are our 5 best-selling products?"
...
def top_products_by_sales(
    ...
    """Rank individual products by total sales and return the top N.

    WHY: Slice and dice both filter by a fixed dimension value.
    A top-N ranking instead orders every member of a dimension
    by a measure and keeps only the strongest performers.
    This answers a very common business question: "Which
    products should we highlight, restock, or promote?"
    ...
    Returns:
        DataFrame with ProductName, Category, and TotalSales
        columns, sorted by TotalSales descending, limited to
        the top_n rows.

    Raises:
        ValueError: If no product sales are found.
    """
    LOG.info(f"OLAP top-N: ranking products by total sales (top {top_n})")
    ...
    df_top_products: pd.DataFrame = conn.execute(sql, [top_n]).df()

    if df_top_products.empty:
        raise ValueError("No product sales were found while ranking top products.")

    LOG.info(f"  Top products returned: {df_top_products.shape[0]}")
...
def summarize(
    ...
    Args:
        ...
        df_top_products: NEW - top-N products ranked by total sales.
    ...
    # NEW: Report the single best-selling product from the
    # top-N ranking (first row, since it is sorted descending).
    top_product_name: str = str(df_top_products.iloc[0]["ProductName"])
    top_product_category: str = str(df_top_products.iloc[0]["Category"])
    top_product_sales: float = float(df_top_products.iloc[0]["TotalSales"])

    LOG.info(
        "Top Product: The best-selling product overall is "
        f"{top_product_name} ({top_product_category}), "
    ...
    LOG.info("Use a top-N ranking to identify leading individual performers.")
    LOG.info("========================")
...
def main() -> None:
    ...
    try:
        ...
        LOG.info("CALL a function to rank top products by sales........")
        df_top_products = top_products_by_sales(
            conn,
            TOP_N_PRODUCTS,
        )

        LOG.info("CALL a function to plot the top-products result........")
        plot_bar(
            df=df_top_products,
            x="ProductName",
            y="TotalSales",
            title=f"Top {TOP_N_PRODUCTS} Products by Total Sales",
            xlabel="Product",
            ylabel="Total Sales ($)",
            palette="Purples_d",
        )
